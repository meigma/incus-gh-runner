...
- Phases 0 through 3 landed through PRs #7, #8, #9, and #10 at `master` commit `d03cace`: the foundation, controller core, reproducible reference VM, one-shot guest contract, and ownership-scoped real Incus lifecycle are in place. GitHub demand/JIT composition remains unwired until phase 4, and both the image and controller lifecycle still need their Incus-capable live gates.
