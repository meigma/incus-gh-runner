...
## 2026-07-21 13:41 — Intended GitHub App deployment
With explicit maintainer authorization, resolved the `catalyst-infra-incus-runners` item in the `Cardano Foundation` 1Password vault. Streamed its valid private key directly into `/etc/incus-gh-runner/github-app-private-key.pem` as `root:root` `0600` without logging or placing the key in command arguments. The installation is active, unsuspended, selected-repository scoped, and grants repository administration write plus metadata read to `cardano-foundation/catalyst-infra` and `cardano-foundation/phoenix`; it does not cover `meigma/incus-gh-runner`. A deliberate first startup against the Meigma repository therefore failed closed with GitHub `403 Resource not accessible by integration` and created no capacity.

Reused unexpired successful Reference Image artifact `8481060937` from merged Phase 4 run `29792916213`; its embedded checksum passed before and after transfer. Imported the Ubuntu 24.04 VM image as `incus-gh-runner-phase5-173381f`, fingerprint `f7a0f28e9914ef475216f9bf6800f08d37582a7956f8ab3ac025a36bb183c411`, into the existing restricted `build-runners` project. Installed the exact branch controller/systemd files, generated a fresh production-shaped TPM enrollment, removed both plaintext copies after origin-key verification, preserved the public key in local ignored evidence, and passed `verify.sh --installed-job-proof=tpm` with `PrivateDevices=yes`.

Retargeted the controller to `cardano-foundation/catalyst-infra`, where the App is intentionally installed. At zero capacity it authenticated, created repository scale set `catalyst-infra-incus-runners-phase5` with scale-set ID `2`, entered its message loop with zero assigned jobs, and created no Incus VM. Stopped the controller cleanly at that boundary; the App key, TPM-encrypted proof credential, enrolled public key, controller configuration, imported image, and persistent GitHub scale set remain ready for the genuine job. Neither permitted repository currently has a safe manual exact-label proof workflow, so adding a temporary workflow to `catalyst-infra` is an explicit cross-repository write requiring maintainer approval.

## 2026-07-21 13:56 — Genuine TPM-backed job proof
With explicit maintainer approval, created the isolated Catalyst Infra branch `feat/incus-runner-phase5-proof` and draft PR #180 containing only a disposable exact-label workflow. Local `moon ci` passed. GitHub workflow run `29867201183` then scheduled on scale set `catalyst-infra-incus-runners-phase5`; GitHub job `88758274268` reported runner ID `25` and runner name `incus-gh-runner-37b77fd3-3f73-4ce0-a66f-9bf318c1feb3`. The controller started the same Incus VM, observed the authenticated `JobStarted` event, emitted the proof, observed successful completion, and deleted the owned VM. Final `build-runners` inventory was empty.

Downloaded artifact `8509715652` (`job-machine-proof-29867201183-1`) into ignored evidence at `build/job-proof-phase5-evidence/20260721-ci-tpm`. External verification with enrolled key ID `sha256:2705f616e4873657feb87eb822ddf00c285ee0f3bab652574f9342b39b3b33eb` passed. Independent GitHub and Incus records matched the proof repository, workflow ref, run ID, runner ID/name, scale-set name, project, VM name/UUID, image fingerprint, profile digest, and launch digest. Wrong-host and tampered-signature checks both failed closed with empty stdout. The service retained `DynamicUser=yes`, `PrivateDevices=yes`, and the two protected systemd credential paths. This completes the required Phase 5 live-proof gate with the same proof schema and verifier behavior as file-backed mode.

Closed Catalyst Infra PR #180 without merge, deleted its remote and local branch, and removed its Worktrunk worktree. Stopped the controller only after owned inventory reached zero, deleted temporary scale set ID `2` after revalidating its exact identity, and removed the controller service, test TPM enrollment, imported test image, config, public key, and binary. Retained only the complete TPM userspace dependency and the authorized GitHub App private key as `root:root` `0600` for the server's intended future use. Draft PR #41 now records the live evidence and stays draft for the human wording/scope review gate. Second-host TPM decryption remains an explicitly untested optional evidence gap; no TPM-native signing, TPM quote, physical-host measurement, or measured-boot claim is made.

## 2026-07-21 14:03 — Close
After maintainer approval, marked PR #41 ready and squash-merged it as `4d891a8` (`feat(provenance): add TPM-bound proof key storage (#41)`). Fast-forwarded local `master` to the same commit, deleted the merged remote and local feature branch, and removed its Worktrunk worktree. Session 020 is complete: Phase 5 is landed and proved on the physical TPM, the disposable Catalyst workflow remains closed without merge, `sre@ci` retains only the approved root-only GitHub App key plus TPM userspace prerequisites, and second-host decryption remains the only explicitly untested optional evidence gap.
