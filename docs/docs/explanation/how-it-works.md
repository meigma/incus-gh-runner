...
Two credentials matter and each is handled to avoid landing on disk or in logs unnecessarily. The GitHub App private key is delivered through a systemd credential (`LoadCredential`) rather than sitting in the configuration file or a general-purpose path, so it inherits systemd's credential-store protections and is not something the controller process has to manage the lifetime of itself. GitHub-side, each VM gets a fresh, single-use JIT runner configuration generated at creation time — never a long-lived registration token — and that configuration is never written to the controller's own logs.
